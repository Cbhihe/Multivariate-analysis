...
# require(graphics)
library(FactoMineR)
library(chemometrics) # needed to invoke the canned algorithm NIPALS: function nipals()
                      # requires package "rparts" or "FactoMineR"
library(ggplot2) # enhanced graphics plotting
library(ggrepel) # to plot observations' projection in PC1-2 with well behaved labeling
...
N <- diag(rep(1,nrow(X))/nrow(X),nrow(X),nrow(X))  # uniform weight matrix
#X_std2 <- scale(N %*% as.matrix(X),center=T,scale=T) # standardize uniformely weighted input data
X_std <- scale(X,T,T) # check that uniform ponderation is assumed by default in scale(X,T,T)
...
colnames(ldg) <- paste0("PC",1:nd) # loadings, XX' eigenvectors in individual obs space R^p
...
LBD <- matrix(0,3,3) ; diag(LBD) <- lbd
evecs_var <- sqrt(N) %*% psi %*% solve(LBD^(0.5))
diag(sqrt(t(ldg[,1:3]) %*% ldg[,1:3]))  # verify that every vectors are normed
